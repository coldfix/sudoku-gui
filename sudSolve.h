...
public:
	...
	enum{ df_maxlev=10,			//default maximum number uf levels
		df_maxunsolved=70,		//maximum number of unsolved fields, when beginning to assume
		df_maxch=10 };			//maximum number of choices in assume-field
	...
	void setsize(const sudSize& s){
		setsize(s.bx(),s.by()); }

	void solve(int maxlev=df_maxlev,Yielder* =0);						// solve until error / ambiguous
	void test(int maxlev=df_maxlev,Yielder* =0);
	void fill(Yielder* =0);
